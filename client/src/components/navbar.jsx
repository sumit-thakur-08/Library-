...
export default function Navbar() {
  ...
  const links = [
    ...
    { name: "Librarians", href: "/librarians" },
  ...
  return (
    <>
            ...
            link.subLinks ? (
              ...
              >
                <div className="flex cursor-pointer items-center gap-1 hover:text-black">
                  {link.name}
                  <ChevronDown
                    className={`mt-px size-4 transition-transform ${
                ...
                <div
                  className={`absolute top-6 left-0 z-40 w-lg rounded-md border border-gray-100 bg-white p-3 shadow-lg transition-all ${
                    ...
                    {link.subLinks.map((sub) => (
                      ...
                      >
                        <div className="rounded-md bg-indigo-600 p-2">
                          <sub.icon className="size-4.5 text-white transition group-hover/link:scale-110" />
                        </div>
                        <div>
      ...
      {/* MOBILE MENU */}
      <div
        className={`fixed inset-0 z-50 flex flex-col items-center justify-center gap-6 bg-white/30 text-lg font-medium backdrop-blur-2xl transition md:hidden ${
      ...
      >
        ...
        <a
          href="pages/login"
          className="rounded-full bg-indigo-600 px-8 py-2.5 font-medium text-white transition hover:opacity-90"
          onClick={() => setIsOpen(false)}
        >
          Login
        </a>
        ...
        <button
          onClick={() => setIsOpen(false)}
          className="rounded-md bg-indigo-600 p-2 text-white active:ring-2"
        >
          <XIcon />
        </button>
      </div>
